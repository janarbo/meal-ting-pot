...
function GetAllChefMenuList() {
    ...
    const updateMenuItemStatusClicked= async(menu_item)=>{
        if (canSave){
            try{
                await updateMenuItemStatus({id: menu_item.menu_item_id,food_type: menu_item.food_type, name: menu_item.name, price: menu_item.price, description:menu_item.description, comment: menu_item.comment, photo: menu_item.photo, spicy_level: menu_item.spicy_level, tags: menu_item.tags, calories: menu_item.calories, ingredients: menu_item.ingredients, status: menu_item.status ? false: true }).unwrap()
                ...
                {menuItems ? (
                    ...
                    {menuItems.map(menu_item => (
                        <tr key={menu_item.menu_item_id}>
                            <td><img src={menu_item.photo} alt="Menu Item Photo" /></td>
