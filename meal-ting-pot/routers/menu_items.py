...
@router.post(
    "/menu-items",
    tags=["MENU ITEMS"],
    response_model=Union[MenuItemOut, Error],
)
...
@router.get(
    "/chef/{chef_id}/menu_items",
    tags=["MENU ITEMS"],
    response_model=Union[List[MenuItemOut], Error],
)
def get_all_chef(
    repo: MenuItemRepository = Depends(),
...
@router.put(
    "/menu-items/{menu_item_id}", response_model=Union[MenuItemOut, Error]
)
